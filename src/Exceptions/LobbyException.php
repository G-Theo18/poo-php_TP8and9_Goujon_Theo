...
{
    public const INVALID_LOBBY      = 1;
    public const ALREADY_IN_LOBBY   = 2;
    ...
    public static function invalidLobby(string $message = 'Invalid lobby'): self
    {
        return new self($message, self::INVALID_LOBBY, 400);
    }

    public static function alreadyInLobby(string $message = 'User already in lobby'): self
    {
        return new self($message, self::ALREADY_IN_LOBBY, 409);
    }

    public static function notFound(string $message = 'Lobby not found'): self
    {
    ...
    public static function fromTrigger(string $message, int $level = E_USER_WARNING): self
    {
        switch ($level) {
            case E_USER_ERROR:
                return new self($message, self::INVALID_LOBBY, 500);
            case E_USER_WARNING:
